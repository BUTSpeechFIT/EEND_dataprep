...
#Script for artificially creating conversations out of telephone conversations (SWITCHBOARD and NIST SRE)
...
SRE_LIST_FILE=$LISTS_DIR/sre.str
SW_LIST_FILE=$LISTS_DIR/sw.str
SWCEL_LIST_FILE=$LISTS_DIR/swcel.str
SEG_LIST_FILE=$LISTS_DIR/swb_sre_comb.srt
SPK_LIST=$LISTS_DIR/swb_sre_comb.speakers
...
SET_DIR=$WORKDIR/$use_rirs$use_noises$use_energies
...
DATA_TEST15h_DIR=$SET_DIR/test15h/data
...
if [ ! -f $SEG_LIST_FILE ]; then
	echo "Processing segments"
	# process segments sre
	grep "sre20" $SEG_FILE | awk '{print $2" "$3" "$4}' | \
		awk -F"-" '{print $1"-"$2"-"$3" "$1"-"$2"-"$3"-"$4"-"$5}' > $SRE_LIST_FILE
	# process segments sw
	grep "sw_" $SEG_FILE | awk '$2 !~ /-/' | awk '{print $2" "$3" "$4}' | \
		awk -F"_" '{print $1"_"$2" "$1"_"$2"_"$3"_"$4"_"$5}' > $SW_LIST_FILE
	# process segments sw cellular
	grep "swbdc" $SEG_FILE | awk '{print $2" "$3" "$4}' | \
		awk -F"-" '{print $1" "$1"-"$2}' > $SWCEL_LIST_FILE
	# join them into one file
	cat $SRE_LIST_FILE $SW_LIST_FILE $SWCEL_LIST_FILE > $SEG_LIST_FILE

	# Gather speakers and split for train and validation
	awk '{print $1}' $SPK2UTT | sort > $SPK_LIST
	shuf --random-source=<(yes 3) $SPK_LIST > $SPK_LIST"_shuf"
	cat $SPK_LIST"_shuf" | head -n $NUMSPKSTEST > $SPK_LIST"_val"
	cat $SPK_LIST"_shuf" | tail -n $(($(wc -l $SPK_LIST"_shuf" | \
		awk '{print $1}') - $NUMSPKSTEST)) > $SPK_LIST"_tr"
	grep -f $SPK_LIST"_val" $SEG_LIST_FILE > $SEG_LIST_FILE"_val"
	grep -f $SPK_LIST"_tr" $SEG_LIST_FILE > $SEG_LIST_FILE"_tr"
...
if [[ ! -d $WAVS_TR_DIR ]]; then
	...
	for f in `find $LISTS_DIR/convtrsublists/conv.lst_*`; do
		echo "python $DIR/conv2wav.py --conversations-list-filename $f \
			--input-wav-scp $WAV_SCP --in-conv-dir $CONV_TR_DIR \
			--out-wav-dir $WAVS_TR_DIR $use_rirs --rirs-wav-scp $RIRS_SCP \
			$use_noises --noises-wav-scp $NOISES_SCP --noises-snrs $SNRS \
			--sampling-frequency $SAMPLINGFREQ" >> $SET_DIR/conv2wav.task
	done
	echo "python $DIR/conv2wav.py --conversations-list-filename $CONV_VAL_LIST \
		--input-wav-scp $WAV_SCP --in-conv-dir $CONV_VAL_DIR \
		--out-wav-dir $WAVS_VAL_DIR $use_rirs --rirs-wav-scp $RIRS_SCP \
		$use_noises --noises-wav-scp $NOISES_SCP --noises-snrs $SNRS \
		--sampling-frequency $SAMPLINGFREQ" >> $SET_DIR/conv2wav.task
...
if [[ ! -d $DATA_VAL_SUBSET_DIR ]]; then
	...
	# Following Hitachi's approach, we take 12.5 hours for validation
	sort $DATA_VAL_DIR/reco2dur | shuf --random-source=<(yes 3) | \
		awk 'BEGIN{sum=0}{sum+=$2 ; if(sum/3600 < 12.5){print $0}}' > $DATA_VAL_SUBSET_DIR/reco2dur
...
if [[ ! -d $DATA_TEST15h_DIR ]]; then
	echo "Defining test set"
	mkdir -p $DATA_TEST15h_DIR

	# Taking 15 hours for test set
	comm -23 <(sort $DATA_VAL_DIR/reco2dur) <(sort $DATA_VAL_SUBSET_DIR/reco2dur) | \
		sort | shuf --random-source=<(yes 3) | \
		awk 'BEGIN{sum=0}{sum+=$2 ; if(sum/3600 < 15){print $0}}' > $DATA_TEST15h_DIR/reco2dur
	
	awk '{print $1}' $DATA_TEST15h_DIR/reco2dur | awk '!seen[$0]++' > $DATA_TEST15h_DIR/recordings

	for ff in rttm segments spk2utt utt2spk wav.scp; do
		grep -f $DATA_TEST15h_DIR/recordings $DATA_VAL_DIR/$ff | \
			awk '!seen[$0]++' > $DATA_TEST15h_DIR/$ff
	done
...
for directory in $DATA_TEST15h_DIR $DATA_VAL_DIR $DATA_VAL_SUBSET_DIR; do
