...
#Scripts for artificially creating conversations out of VoxCeleb
...
if [ ! -f $LIST_FILE_TR ]; then
    ...
    grep -F -f <(awk '{print $1}' $UTT2SPK_TR) <(awk '{print $2" "$3" "$4}' $SEG_FILE) | awk -F"/" '{print $1"/"$2"/"$3"/"$4" "$0}' | tr / _ > $LIST_FILE_TR
    # process segments validation
    grep -F -f <(awk '{print $1}' $UTT2SPK_VAL) <(awk '{print $2" "$3" "$4}' $SEG_FILE) | awk -F"/" '{print $1"/"$2"/"$3"/"$4" "$0}' | tr / _ > $LIST_FILE_VAL
